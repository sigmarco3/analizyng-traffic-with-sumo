...
def plotResult(file,file2):
       ...
       # plt.plot(x, ym)
        ...
        sc1 = np.arange(10)
        sc2 = np.arange(10)
        time = np.arange(10000,110000,10000)
        for i in range(0,len(m_v)):
            m_v[i]=np.mean(tm1[(i*2000):((i+1)*2000)])
            zc1[i]=np.mean(zm1[(i * 2000):((i + 1) * 2000)])
            uc1[i] = np.mean(um1[(i * 2000):((i + 1) * 2000)])
            vc1[i] = np.mean(vm1[(i * 2000):((i + 1) * 2000)])
            ar1[i] = np.mean(am1[(i * 2000):((i + 1) * 2000)])
            sc1[i] = np.mean(sm1[(i * 2000):((i + 1) * 2000)])
        ...
        plt.errorbar(time,z1,yerr=std_error,marker="o",color="blue",markersize=2, label="Q-learning")  #auto ferme su auto circolanti in funzione del tempo
        ...
        for i in range(0, len(m_veh) ):
            m_veh[i] = np.mean(tm2[(i * 2000):((i + 1) * 2000)])

            zc2[i]=np.mean(zm2[(i * 2000):((i + 1) * 2000)])
            uc2[i] = np.mean(um2[(i * 2000):((i + 1) * 2000)])
            vc2[i] = np.mean(vm2[(i * 2000):((i + 1) * 2000)])
            ar2[i] = np.mean(am2[(i * 2000):((i + 1) * 2000)])
            sc2[i] = np.mean(sm2[(i * 2000):((i + 1) * 2000)])
        ...
        plt.title("rete 4x4 reward 'diff-waiting-time' e semafori standard misura total stopped/running veicles ", size=15)
        plt.ylabel("system total stopped/running vehicles", size=15)
       ...
       #throughtput
        ...
        plt.errorbar(time, tput1, yerr=std_error, marker="o", color="blue", markersize=5, label="Q-learning")
        tput2 = ar2/sc2
        ...
        plt.xlabel("time-step", size=15)
        plt.xticks(time, time)
        plt.title("rete 4x4 reward 'diff-waiting-time' e semafori standard misura troughput",size=15)
        plt.ylabel("system throughput", size=15)
        plt.legend()
        plt.grid()
        plt.show()
        #mean waiting time
        ...
        # create chart
        plt.errorbar(time,uc1,yerr=std_error,marker="o",color="blue",markersize=2, label="Q-learning")
        ...
        plt.xlabel("time-step",size=15)
        plt.title("rete 4x4 reward 'diff-waiting-time' e semafori standard mean waiting time ",size=15)
        plt.ylabel("system mean waiting time(seconds)",size=15)
        plt.xticks(time,time)
        plt.legend()
        ...
        std_error = np.std(vc1, ddof=1) / np.sqrt(len(vc1))
        # create chart
        plt.errorbar(time, vc1, yerr=std_error, marker="o",markersize=5, color="blue", label="Q-learning ")
        ...
        #     label = int(x)
            ...
            #              xytext=(0, 5),  # distance from text to points (x,y)
            #              ha='center')  # horizontal alignment can be left, right or center
        std_error = np.std(vc2, ddof=1) / np.sqrt(len(vc2))
        plt.errorbar(time, vc2, yerr=std_error, marker="x", color="orange", markersize=2,label="semaforo standard")
        plt.xlabel("time-step ",size=15)
        plt.title("rete 4x4 reward 'diff-waiting-time' e semafori standard mean speed", size =15)
        plt.legend()
        plt.xticks(time, time)
        plt.grid()
        plt.ylabel("system mean speed(m/s)",size =15)
        plt.show()
if __name__ == '__main__':
        file2 = 'D:/programmi/sumo/esperimenti semafori/Reinforcement-learning-in-traffic-light/outputs/4x4/result-4x4 random-static(100mila)'
        plotResult('D:/programmi/sumo/esperimenti semafori/Reinforcement-learning-in-traffic-light/outputs/4x4/result_trainingSingoli_crescente(wait)1M',file2)