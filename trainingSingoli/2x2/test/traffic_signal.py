...
class TrafficSignal:
    ...
    def compute_reward(self):
        if type(self.reward_fn) is str:
            ...
            elif self.reward_fn =='max':
                self.last_reward == self._diff_max_queue()
            else:
    ...
    def _diff_waiting_time_reward(self):
        ...
        return reward
    def _diff_max_queue(self):
        max = self.get_max_queue()
        reward = self.last_max - max
        return reward
    ...
    def get_max_queue(self):
        veh_list = []
        for lane in self.lanes:
            veh_list[i]= self.sumo.lane.getLastStepVehicleNumber(lane)
        return np.max(veh_list)