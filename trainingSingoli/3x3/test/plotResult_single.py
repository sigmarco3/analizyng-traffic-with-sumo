...
def plotResult(file,file2):
    #with open(file + '_conn0_run4.csv', newline="", encoding="ISO-8859-1") as filecsv:
       ...
       # plt.plot(x, ym)
        ...
        width = 7
        ind = np.arange(10)
        plt.errorbar(m_v,zc1,yerr=std_error,marker="o",markersize=2,color="blue",label="training copiato")
        ...
        plt.title("rete 3x3 traffico crescente reward 'average-speed' misura total stopped ")
        ...
        print(autoMedie)
        print(m_v)
        plt.errorbar(m_veh,zc2,yerr=std_error,marker="x",markersize=2,color="orange",label="training indipendente")
        ...
        # create chart
        plt.errorbar(m_v,uc1,yerr=std_error,marker="x",markersize=2,color="blue", label="training copiato")
        ...
        # create chart
        plt.errorbar(m_veh,uc2,yerr=std_error,marker="o",markersize=2,color="orange",label="training indipendente")
        print("valor medio di mean waiting time :" ,uc2.mean())


        print("massimo di mean waiting time :", uc2.max())
        plt.xlabel("numero auto medio in circolazione ogni 1000 secondi",size=12)
        plt.title("rete 3x3 traffico crescente reward 'average-speed' misura mean waiting time ")
        ...
        plt.errorbar(m_v, vc2, yerr=std_error, marker="x", markersize=2,color="orange", label="training indipendente")
        plt.xlabel("auto")
        plt.title("3x3 percorsi traffico crescente reward 'average-speed' misura mean speed")
        plt.legend()
        plt.xticks(autoMedie, autoMedie)
        plt.grid()
...
if __name__ == '__main__':
    file2='D:/programmi/sumo/esperimenti semafori/Reinforcement-learning-in-traffic-light/outputs/3x3/result-alpha0.1-gamma0.99_trainingSingoli_crescente(avg)corretto'
    plotResult('D:/programmi/sumo/esperimenti semafori/Reinforcement-learning-in-traffic-light/outputs/3x3/result_trainingCopiato_crescente(avg)corretto',file2)